%% Function to generate postanoxic EEG using the model by Ruijter et al, 2017
% Reference: BJ Ruijter, J Hofmeijer, HGE Meijer, MJAM van Putten. 
% Synaptic damage underlies EEG abnormalities in postanoxic encephalopathy:
% a computational study. Clinical Neurophysiology, 2017.
%
% Inputs:   tau_rec_e:  recovery time for excitatory synapses
%           tau_rec_i:  recovery time for inhibitory synapses
%           ltp_factor: strength of long-term-potentiation
%           propofol:   concentration 'c' of propofol
%
% Ouputs:   none (only a 10-second sample of simulated EEG is plotted)
%
% If no input arguments are provided, the script uses a predefined set of
% variables to generate a sample EEG pattern for one of the categories 
% defined in Figure 2. Another category can be selected by changing the 
% value of "parset_no" in line 52 of the script.
%
% Note :    Some of the variable names in this script differ from those used
%           in the article. The variable names in this script match those
%           used in: Liley et al, A spatially continuous mean field theory 
%           of electrocortical activity, Network 13(1):67{113, 2002.
%
%           Article (k=e,i):            Script (k=e,i):
%           V_k                         h_k
%           V_k^r                       h_k_r
%           V_k^eq                      h_kl_eq
%           Q_k^max                     S_k^max
%
%           Besides, the script allows for postsynaptic dependency in
%           some variables, where the article doesn't. For example, in the
%           article we use 'Gamma_e', whereas the script uses separate 
%           variables 'Gamma_ee' and 'Gamma_ei'.
%
% created by:   Barry Ruijter
% last updated: 26 April 2017
%
%% start code

function Ruijter2017_model(tau_rec_e,tau_rec_i,ltp_factor,propofol)

	% If no input arguments are provided: choose predefined set
    if nargin==0
	       
        % Choose one of the following values for "parset_no":
        % 1: normal EEG
        % 2: low voltage
        % 3: discontinuous
        % 4: burst-suppression
        % 5: irregular discharges
        % 6: periodic discharges
          
		parset_no=4;
        
        parset = load_parset(parset_no);
		
		tau_rec_e	=	parset.tau_rec_e;
		tau_rec_i	=	parset.tau_rec_i;
		ltp_factor	=	parset.ltp_factor;
		propofol	=	parset.propofol;
        titletext	=	parset.titletext;
        ylimits		=	parset.ylimits;
    end

    % 0. General settings
    sim_time=25;            % simulation time (sec)
    steps=10000;            % simulation steps per second / 'sample frequency' (Hz)
    ...
    EEG = downsample(EEG,40);   
	fs=250;						% sample frequency after downsampling   
	EEG = EEG((15*fs):end); 	% remove transient effects 
	filtOrder=3;          
    filtFreq= [0.5 30];     
    [b,a]= butter(filtOrder,filtFreq/(0.5*fs),'bandpass'); 
    EEG_filtered = filtfilt(b,a,EEG); % apply bandpass filter
  
    % 8. plot results
    time=0:(1/fs):(length(EEG_filtered)-1)/fs;
    range= time<10; % plot only the first 10 seconds
   
    figure
    plot(time(range),EEG_filtered(range))
    if nargin==0
        title(titletext)
        ylim(ylimits);
    end
    ...
    function parset=load_parset(parset_no)
        switch parset_no
            case 1
                parset.tau_rec_e=500;
                parset.tau_rec_i=500;
                parset.ltp_factor=0;
                parset.propofol=0;
                parset.titletext='Example 1: normal EEG';
                parset.ylimits=[-3 3];
            case 2
                parset.tau_rec_e=100000;
                parset.tau_rec_i=1800;
                parset.ltp_factor=0;
                parset.propofol=0;
                parset.titletext='Example 2: low voltage';
                parset.ylimits=[-3 3];             
            case 3
                parset.tau_rec_e=27000;
                parset.tau_rec_i=2300;
                parset.ltp_factor=0;
                parset.propofol=0;
                parset.titletext='Example 3: discontinuous';
                parset.ylimits=[-3 3];              
            case 4
                parset.tau_rec_e=5000;
                parset.tau_rec_i=2500;
                parset.ltp_factor=3;
                parset.propofol=0;
                parset.titletext='Example 4: burst-suppression';
                parset.ylimits=[-60 40];               
            case 5
                parset.tau_rec_e=13000;
                parset.tau_rec_i=800;
                parset.ltp_factor=0.8;
                parset.propofol=0;
                parset.titletext='Example 5: irregular discharges';
                parset.ylimits=[-60 40];                   
            case 6
                parset.tau_rec_e=1500;
                parset.tau_rec_i=2000;
                parset.ltp_factor=0.8;
                parset.propofol=0;
                parset.titletext='Example 6: periodic discharges';
                parset.ylimits=[-60 40];                     
        end
